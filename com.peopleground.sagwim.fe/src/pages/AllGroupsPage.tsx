...
export function AllGroupsPage() {
  ...
  const [dong, setDong] = useState<string>('우리 동네')
  ...
  const loadPage = useCallback(
    async (pageNum: number, append: boolean) => {
      try {
        ...
        const response = await getGroups(token, pageNum, PAGE_SIZE)
  ...
  useEffect(() => {
    getMyProfile(token)
      .then((profile) => setDong(extractLastRegionToken(profile.address) ?? '우리 동네'))
      .catch(() => {})
  }, [token])
    ...
    if (groups.length === 0) {
      return (
        <div className={styles.emptyStateCenter}>
          <EmptyState
            title="노출 범위 내 모임이 아직 없어요."
            description="주변 모임이 생기면 여기에 표시됩니다."
          />
        </div>
      )
    }
        ...
        <div className={styles.pageHeader}>
          ...
          <div>
            <h1 className={styles.pageTitle} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <img src={pinIcon} alt="" width={22} height={22} />
              {dong} 모든 모임
            </h1>
            <p className={styles.pageSubtitle}>우리 동네 모임 전체 목록</p>
          </div>
        </div>
