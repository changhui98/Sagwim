...
import { getDeadlineGroups, toggleGroupLike } from '../api/groupApi'
...
export function DeadlineGroupsPage() {
  ...
  const loadPage = useCallback(
    async (pageNum: number, append: boolean) => {
      try {
        ...
        const response = await getDeadlineGroups(token, pageNum, PAGE_SIZE)
    ...
    if (groups.length === 0) {
      return (
        <div className={styles.emptyStateCenter}>
          <EmptyState
            title="마감 임박 모임이 없습니다."
            description="곧 자리가 사라질 모임이 여기에 표시됩니다."
          />
        </div>
      )
    }
        ...
        <div className={styles.pageHeader}>
          ...
          <div>
            <h1 className={styles.pageTitle} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <img src={deadlineIcon} alt="" width={22} height={22} />
              마감 임박 모임
            </h1>
            <p className={styles.pageSubtitle}>정원이 거의 다 찬 모임 전체 목록</p>
          </div>
        </div>
